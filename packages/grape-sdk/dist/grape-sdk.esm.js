...
import { ZERO, toElastic, WNATIVE_ADDRESS, getProviderOrSigner, BENTOBOX_ADDRESS, KASHI_ADDRESS, JSBI as JSBI$1, minimum, maximum } from '@champagneswap/core-sdk';
import { AddressZero } from '@ethersproject/constants';
import { Contract } from '@ethersproject/contracts';
import { defaultAbiCoder } from '@ethersproject/abi';
import { toShare, toAmount } from '@champagneswap/cellar-sdk';
...
var KashiCooker = /*#__PURE__*/function () {
  ...
  _proto.bentoDepositCollateral = function bentoDepositCollateral(amount) {
    var useNative = this.pair.collateral.address === WNATIVE_ADDRESS[this.chainId];
  ...
  _proto.addAsset = function addAsset(amount, fromBento) {
    ...
    if (fromBento) {
      ...
      var useNative = this.pair.asset.address === WNATIVE_ADDRESS[this.chainId];
  ...
  _proto.repayPart = function repayPart(part, fromBento) {
    if (!fromBento) {
      var useNative = this.pair.asset.address === WNATIVE_ADDRESS[this.chainId];
      this.add(KashiAction.GET_REPAY_SHARE, defaultAbiCoder.encode(['int256'], [part]));
      this.add(KashiAction.BENTO_DEPOSIT, defaultAbiCoder.encode(['address', 'address', 'int256', 'int256'], [useNative ? AddressZero : this.pair.asset.address, this.account, 0, -1]), // TODO: Put some warning in the UI or not allow repaying ETH directly from wallet, because this can't be pre-calculated
      useNative ? JSBI.divide(JSBI.multiply(toShare(this.pair.asset, toElastic(this.pair.totalBorrow, part, true)), JSBI.BigInt(1001)), JSBI.BigInt(1000)) : ZERO);
    }
      ...
      return runtime_1.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            ...
            case 2:
              kashiPairCloneContract = new Contract(this.pair.address, KASHIPAIR_ABI, getProviderOrSigner(this.library, this.account));
              _context.prev = 3;
              _context.next = 6;
...
var computePairAddress = function computePairAddress(_ref) {
  ...
  return getCreate2Address(BENTOBOX_ADDRESS[collateral.chainId], keccak256(['bytes'], [defaultAbiCoder.encode(['address', 'address', 'address', 'bytes'], [collateral.address, asset.address, oracle, oracleData])]), keccak256(['bytes'], ['0x3d602d80600a3d3981f3363d3d373d3d3d363d73' + KASHI_ADDRESS[collateral.chainId].substring(2) + '5af43d82803e903d91602b57fd5bf3']));
};
...
function accrue(pair, amount, includePrincipal) {
  ...
  return JSBI$1.add(JSBI$1.divide(JSBI$1.multiply(JSBI$1.multiply(amount, pair.accrueInfo.interestPerSecond), pair.elapsedSeconds), JSBI$1.BigInt(1e18)), includePrincipal ? amount : ZERO);
}
function accrueTotalAssetWithFee(pair) {
  var extraAmount = JSBI$1.divide(JSBI$1.multiply(JSBI$1.multiply(pair.totalBorrow.elastic, pair.accrueInfo.interestPerSecond), JSBI$1.add(pair.elapsedSeconds, JSBI$1.BigInt(3600)) // For some transactions, to succeed in the next hour (and not only this block), some margin has to be added
  ), JSBI$1.BigInt(1e18));
  var feeAmount = JSBI$1.divide(JSBI$1.multiply(extraAmount, PROTOCOL_FEE), PROTOCOL_FEE_DIVISOR); // % of interest paid goes to fee

  var feeFraction = JSBI$1.divide(JSBI$1.multiply(feeAmount, pair.totalAsset.base), pair.currentAllAssets);
  return {
    elastic: pair.totalAsset.elastic,
    base: JSBI$1.add(pair.totalAsset.base, feeFraction)
  };
}
function interestAccrue(pair, interest) {
  if (JSBI$1.equal(pair.totalBorrow.base, ZERO)) {
    return STARTING_INTEREST_PER_YEAR;
  }

  if (JSBI$1.lessThanOrEqual(pair.elapsedSeconds, ZERO)) {
    return interest;
  }

  var currentInterest = interest;

  if (JSBI$1.lessThan(pair.utilization, MINIMUM_TARGET_UTILIZATION)) {
    var underFactor = JSBI$1.greaterThan(MINIMUM_TARGET_UTILIZATION, ZERO) ? JSBI$1.divide(JSBI$1.multiply(JSBI$1.subtract(MINIMUM_TARGET_UTILIZATION, pair.utilization), FACTOR_PRECISION), MINIMUM_TARGET_UTILIZATION) : ZERO;
    var scale = JSBI$1.add(INTEREST_ELASTICITY, JSBI$1.multiply(JSBI$1.multiply(underFactor, underFactor), pair.elapsedSeconds));
    currentInterest = JSBI$1.divide(JSBI$1.multiply(currentInterest, INTEREST_ELASTICITY), scale);

    if (JSBI$1.lessThan(currentInterest, MINIMUM_INTEREST_PER_YEAR)) {
      currentInterest = MINIMUM_INTEREST_PER_YEAR; // 0.25% APR minimum
    }
  } else if (JSBI$1.greaterThan(pair.utilization, MAXIMUM_TARGET_UTILIZATION)) {
    var overFactor = JSBI$1.multiply(JSBI$1.subtract(pair.utilization, MAXIMUM_TARGET_UTILIZATION), JSBI$1.divide(FACTOR_PRECISION, FULL_UTILIZATION_MINUS_MAX));

    var _scale = JSBI$1.add(INTEREST_ELASTICITY, JSBI$1.multiply(JSBI$1.multiply(overFactor, overFactor), pair.elapsedSeconds));

    currentInterest = JSBI$1.divide(JSBI$1.multiply(currentInterest, _scale), INTEREST_ELASTICITY);

    if (JSBI$1.greaterThan(currentInterest, MAXIMUM_INTEREST_PER_YEAR)) {
...
function takeFee(amount) {
  return JSBI$1.subtract(amount, JSBI$1.divide(JSBI$1.multiply(amount, PROTOCOL_FEE), PROTOCOL_FEE_DIVISOR));
}
...
var KashiMediumRiskLendingPair = /*#__PURE__*/function () {
  ...
  _createClass(KashiMediumRiskLendingPair, [{
    key: "elapsedSeconds",
    get: function get() {
      var currentDate = JSBI$1.divide(JSBI$1.BigInt(Date.now()), JSBI$1.BigInt(1000));
      return JSBI$1.subtract(currentDate, this.accrueInfo.lastAccrued);
    }
    /**
    ...
    key: "interestPerYear",
    get: function get() {
      return JSBI$1.multiply(this.accrueInfo.interestPerSecond, JSBI$1.BigInt(60 * 60 * 24 * 365));
    }
    /**
    ...
    get: function get() {
      return JSBI$1.add(this.totalAssetAmount, this.currentBorrowAmount);
    }
    /**
    ...
    get: function get() {
      return minimum(JSBI$1.divide(JSBI$1.multiply(this.totalAsset.elastic, this.currentAllAssets), this.currentAllAssetShares), toAmount(this.asset, toElastic(this.currentTotalAsset, JSBI$1.subtract(this.totalAsset.base, JSBI$1.BigInt(1000)), false)));
    }
    /**
    ...
    get: function get() {
      return JSBI$1.divide(JSBI$1.multiply(this.maxAssetAvailable, this.currentTotalAsset.base), this.currentAllAssets);
    }
    /**
    ...
    key: "marketHealth",
    get: function get() {
      return JSBI$1.divide(JSBI$1.multiply(JSBI$1.divide(JSBI$1.multiply(this.totalCollateralAmount, JSBI$1.BigInt(1e18)), maximum(this.exchangeRate, this.spotExchangeRate, this.oracleExchangeRate)), JSBI$1.BigInt(1e18)), this.currentBorrowAmount);
    }
    /**
    ...
    key: "utilization",
    get: function get() {
      return JSBI$1.divide(JSBI$1.multiply(JSBI$1.BigInt(1e18), this.currentBorrowAmount), this.currentAllAssets);
    }
    /**
    ...
    key: "supplyAPR",
    get: function get() {
      return takeFee(JSBI$1.divide(JSBI$1.multiply(this.interestPerYear, this.utilization), JSBI$1.BigInt(1e18)));
    }
    /**
    ...
    get: function get() {
      return takeFee(JSBI$1.divide(JSBI$1.multiply(this.currentInterestPerYear, this.utilization), JSBI$1.BigInt(1e18)));
    }
    /**
    ...
    get: function get() {
      return JSBI$1.divide(JSBI$1.multiply(this.userAssetFraction, this.currentAllAssets), this.totalAsset.base);
    }
    /**
    ...
    get: function get() {
      if (JSBI$1.equal(this.userBorrowPart, ZERO)) return ZERO;
      return JSBI$1.divide(JSBI$1.multiply(this.userBorrowPart, this.currentBorrowAmount), this.totalBorrow.base);
    }
    /**
    ...
    get: function get() {
      return JSBI$1.divide(JSBI$1.multiply(this.userAssetFraction, this.currentBorrowAmount), this.totalAsset.base);
    }
    /**
    ...
    key: "feesEarned",
    get: function get() {
      return JSBI$1.divide(JSBI$1.multiply(this.accrueInfo.feesEarnedFraction, this.currentAllAssets), this.totalAsset.base);
    }
    /**
    ...
    get: function get() {
      var max = {
        oracle: JSBI$1.divide(JSBI$1.multiply(this.userCollateralAmount, JSBI$1.multiply(JSBI$1.BigInt(1e16), JSBI$1.BigInt(75))), this.oracleExchangeRate),
        spot: JSBI$1.divide(JSBI$1.multiply(this.userCollateralAmount, JSBI$1.multiply(JSBI$1.BigInt(1e16), JSBI$1.BigInt(75))), this.spotExchangeRate),
        stored: JSBI$1.divide(JSBI$1.multiply(this.userCollateralAmount, JSBI$1.multiply(JSBI$1.BigInt(1e16), JSBI$1.BigInt(75))), this.exchangeRate)
      };
      var min = minimum.apply(void 0, Object.values(max));
      var safe = JSBI$1.subtract(JSBI$1.divide(JSBI$1.multiply(min, JSBI$1.BigInt(95)), JSBI$1.BigInt(100)), this.currentUserBorrowAmount);
      var possible = minimum(safe, this.maxAssetAvailable);
      return _extends({}, max, {
        minimum: min,
        safe: safe,
        possible: possible
    ...
    key: "health",
    get: function get() {
      return JSBI$1.divide(JSBI$1.multiply(this.currentUserBorrowAmount, JSBI$1.BigInt(1e18)), this.maxBorrowable.minimum);
    }
  }]);
...
export { ACTION_ACCRUE, ACTION_ADD_ASSET, ACTION_ADD_COLLATERAL, ACTION_BENTO_DEPOSIT, ACTION_BENTO_SETAPPROVAL, ACTION_BENTO_TRANSFER, ACTION_BENTO_TRANSFER_MULTIPLE, ACTION_BENTO_WITHDRAW, ACTION_BORROW, ACTION_CALL, ACTION_GET_REPAY_PART, ACTION_GET_REPAY_SHARE, ACTION_REMOVE_ASSET, ACTION_REMOVE_COLLATERAL, ACTION_REPAY, ACTION_UPDATE_EXCHANGE_RATE, FACTOR_PRECISION, FULL_UTILIZATION, FULL_UTILIZATION_MINUS_MAX, INTEREST_ELASTICITY, KashiAction, KashiCooker, KashiMediumRiskLendingPair, MAXIMUM_INTEREST_PER_YEAR, MAXIMUM_TARGET_UTILIZATION, MINIMUM_INTEREST_PER_YEAR, MINIMUM_TARGET_UTILIZATION, PROTOCOL_FEE, PROTOCOL_FEE_DIVISOR, STARTING_INTEREST_PER_YEAR, UTILIZATION_PRECISION, accrue, accrueTotalAssetWithFee, addBorrowFee, computePairAddress, interestAccrue, takeFee };
//# sourceMappingURL=grape-sdk.esm.js.map
